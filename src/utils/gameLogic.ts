  ...
  if (cards.length >= 6 && cards.length % 2 === 0) {
    ...
    if (pairs.length === cards.length / 2) {
      ...
      for (let i = 1; i < pairs.length; i++) {
        if (RANK_VALUES[pairs[i][0].rank] !== RANK_VALUES[pairs[i - 1][0].rank] + 1) {
          isConsecutive = false;
          break;
        }
      }
...
export function canBeat(newPlay: Combination, currentPlay: Combination | null): boolean {
  // If no current play, anything is valid
  if (!currentPlay) return true;

  // Four of a kind can beat any single 2
  if (newPlay.type === 'four-of-a-kind' && 
  ...
  if (newPlay.type === 'double-sequence' && 
  ...
  if ((newPlay.type === 'straight' || newPlay.type === 'double-sequence') &&
...
export function findAIPlay(
  ...
  if (mustIncludeThreeOfSpades) {
    ...
    if (threeOfSpades) {
      // Try to play a combination with 3 of spades
      const otherThrees = hand.filter(c => c.rank === '3' && c.suit !== 'spades');
      if (otherThrees.length >= 1) {
        // Play pair of 3s
        return [threeOfSpades, otherThrees[0]];
      }
      // Just play single 3 of spades
      return [threeOfSpades];
    }
  }

  // No current play - AI can play anything (prefer lowest cards)
  if (!currentPlay) {
    // Play lowest single
    return [hand[0]];
  }
